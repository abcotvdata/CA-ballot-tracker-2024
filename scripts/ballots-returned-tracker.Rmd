...
vbm_ballots_issued = sum(ballots_clean$`VBM Ballots issued`)
vbm_ballots_returned = sum(ballots_clean$`VBM ballots returned`)
vbm_ballots_accepted = sum(ballots_clean$`VBM Ballots accepted`)
#total_pct_returned = round(((total_ballots_returned/total_ballots_issued)*100), digits=1)
person_ballots_accepted = sum(ballots_clean$`In Person Ballots accepted`)
total_accepted = sum(ballots_clean$`VBM and In Person Ballots Accepted`)
...
Mail-in and in-person voting is underway in California for the Nov. 5, 2024, General Election, and the number of ballots cast is climbing each day. Click on or hover over each county to see the percent of registered voters who have already voted as of `r today_display`.
